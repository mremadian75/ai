...
final class VES_Intelligence_Store {
    ...
    public static function find_insight_by_trend_record(int $workspace_id, int $trend_record_id): int {
        ...
        // Match the id with a JSON value boundary (followed by ',' or '}') so trend
        // record 12 does NOT collide with 123 (prefix match). Metadata is compact JSON,
        // so the value is always terminated by ',' or '}'.
        $n = (int) $trend_record_id;
        return (int) $wpdb->get_var($wpdb->prepare(
            'SELECT id FROM ' . self::table_name('insight') . " WHERE workspace_id = %d AND (metadata LIKE %s OR metadata LIKE %s) ORDER BY id ASC LIMIT 1",
            $workspace_id, '%"trend_record_id":' . $n . ',%', '%"trend_record_id":' . $n . '}%'
        ));
    }
