...
final class FIDTF_Report_Service {
    ...
    private static function recommended_channel_to_test(array $source_summary, array $formats): string {
        ...
        $format = self::label_from_ranked($formats, 0, 'short-form test');
        if ($has_tiktok) { return 'TikTok / short-form creative mechanics (' . $format . ')'; }
        if ($has_instagram) { return 'Instagram / visual occasion and caption language'; }
    ...
    private static function executive_summary(array $run, int $count, array $source_summary, array $terms, array $hashtags, array $formats, array $stats = [], array $intel = []): string {
        ...
        $read = 'The run found ' . $count . ' usable public signals from ' . $source_count . ' source family/families' . $market_part . $topic_part . '. ';
        if ($direct > 0) {
            $read .= $direct . ' signals directly match the core request and ' . $adjacent . ' are adjacent cultural/context signals. ';
        } else {
    ...
    private static function evidence_fit_diagnosis(array $items, array $source_summary, array $focus, array $intel, array $stats): array {
        ...
        $diagnosis = [];
        $diagnosis['core_request_fit'] = $direct > 0
            ? $direct . ' of ' . $total . ' stored evidence rows directly match the core request.'
            : 'No stored evidence row directly matches the core request strongly enough; the run is currently category/context-led.';
        if ($brand !== '') {
            $diagnosis['brand_fit'] = $brand_direct > 0
                ? $brand_direct . ' row(s) explicitly mention ' . $brand . '. Treat the rest as category or cultural context.'
        ...
        $diagnosis['context_fit'] = $dominant
            ...
            : 'Direct evidence is strong enough to guide the first validation brief, with adjacent signals as support.';
        $diagnosis['coverage'] = $source_families . ' source family/families contain evidence. Cross-source confirmation is required before client-facing certainty.';
        $diagnosis['quality_gate'] = 'Use direct rows for claims, adjacent rows for tone and format inspiration, and weak rows only as background.';
        return $diagnosis;
    }

    private static function signal_clusters(array $items, array $focus): array {
        $clusters = [
            'brand_direct' => ['label' => 'Brand-direct evidence', 'count' => 0, 'examples' => []],
            'category_context' => ['label' => 'Category/context evidence', 'count' => 0, 'examples' => []],
            'occasion_social' => ['label' => 'Occasion and social behavior', 'count' => 0, 'examples' => []],
            'humor_meme' => ['label' => 'Humor / meme mechanics', 'count' => 0, 'examples' => []],
            'format_hook' => ['label' => 'Reusable hook / format mechanics', 'count' => 0, 'examples' => []],
        ];
        $brand_terms = self::clean_terms([(string) ($focus['brand'] ?? ''), (string) ($focus['company'] ?? '')], 6);
        $category_terms = self::clean_terms(array_merge((array) ($focus['category_terms'] ?? []), ['beer','cerveza','beverage','drink','drinks','bebida','bebidas']), 16);
        foreach ($items as $item) {
            $text = self::item_plain_text($item);
            $title = self::short_text((string) (($item['title'] ?? '') ?: ($item['caption_or_text'] ?? 'Evidence item')), 110);
            if (!empty($brand_terms) && self::text_has_any($text, $brand_terms)) { self::add_cluster_example($clusters['brand_direct'], $title); }
            if (!empty($category_terms) && self::text_has_any($text, $category_terms)) { self::add_cluster_example($clusters['category_context'], $title); }
            if (preg_match('/fiesta|party|celebr|night|noche|amigos|friends|summer|verano|festival|bar|terrace|terraza/u', $text)) { self::add_cluster_example($clusters['occasion_social'], $title); }
            if (preg_match('/meme|😂|🤣|humor|joke|funny|viral|trend|challenge|plantilla/u', $text)) { self::add_cluster_example($clusters['humor_meme'], $title); }
            if (preg_match('/pov|cuando|when|how|como|receta|recipe|storytime|hook|template|before|after/u', $text)) { self::add_cluster_example($clusters['format_hook'], $title); }
        }
        foreach ($clusters as $key => $cluster) {
            if ((int) ($cluster['count'] ?? 0) <= 0) { unset($clusters[$key]); }
        }
    ...
    private static function source_confidence(array $source_summary, array $quality): array {
        ...
        return [
            'level' => $level,
            'reason' => $families . ' source family/families with relevant evidence; ' . $decision_sources . ' source family/families with decision-ready evidence; ' . $rows . ' relevant stored rows. ' . (string) ($quality['summary'] ?? ''),
        ];
    }
    ...
    private static function evidence_quality_summary(array $items, array $source_summary, array $stats): array {
        ...
        $summary = $count . ' relevant rows; ' . $source_count . ' source family/families; ' . count($decision_sources) . ' source family/families with decision-ready evidence; ' . $has_metrics . ' rows with metrics; ' . $has_url . ' rows with source URLs. Decision-ready rows: ' . $decision_ready . '; creative-only rows: ' . (int) $tier_counts['creative_only'] . '; weak/noise rows: ' . (int) ($tier_counts['weak'] + $tier_counts['noise']) . '.';
    ...
    private static function source_summary(array $jobs, array $items): array {
        ...
        foreach ($items as $item) {
            ...
            if (!isset($tier_counts_by_source[$source])) {
                $tier_counts_by_source[$source] = ['decision_ready_count' => 0, 'creative_only_count' => 0, 'weak_or_noise_count' => 0];
            }
            $tier = sanitize_key((string) ($item['evidence_tier'] ?? 'weak'));
            $use = sanitize_key((string) ($item['recommended_use'] ?? ''));
            ...
            if (in_array($tier, ['strong','support'], true) && $is_report_relevant) { $tier_counts_by_source[$source]['decision_ready_count']++; }
            elseif ($tier === 'creative_only' && $is_report_relevant) { $tier_counts_by_source[$source]['creative_only_count']++; }
            else { $tier_counts_by_source[$source]['weak_or_noise_count']++; }
            if (!isset($summary[$source])) {
                $summary[$source] = ['status' => 'ingested', 'raw_count' => 0, 'normalized_count' => 0, 'relevant_count' => 0, 'provider_dataset_rows' => 0, 'provider_dataset_total_rows' => 0, 'flattened_raw_items' => 0, 'error_code' => ''];
            }
        }
        foreach ($item_counts as $source => $count) {
            $summary[$source]['relevant_count'] = max((int) ($summary[$source]['relevant_count'] ?? 0), (int) $count);
            foreach (['decision_ready_count','creative_only_count','weak_or_noise_count'] as $metric) {
                $summary[$source][$metric] = (int) ($tier_counts_by_source[$source][$metric] ?? 0);
            }
        }
        return $summary;
    }
    ...
    private static function statistical_summary(array $source_summary, array $items): array {
        ...
        return [
            'total_raw_items' => array_sum(array_map(static function($row) { return (int) ($row['raw_count'] ?? 0); }, $by_source)),
    ...
    private static function sources_with_evidence(array $source_summary): array {
        $out = [];
        foreach ($source_summary as $source => $summary) {
            if ((int) ($summary['relevant_count'] ?? 0) > 0) { $out[] = (string) $source; }
        }
        return $out;
    }
    ...
    private static function sources_with_provider_rows(array $source_summary): array {
        $out = [];
        foreach ($source_summary as $source => $summary) {
            $rows = max((int) ($summary['provider_dataset_rows'] ?? 0), (int) ($summary['raw_count'] ?? 0));
    ...
    private static function top_terms(array $items, int $limit = 10): array {
        ...
        $counts = self::token_counts(implode(' ', $text));
    ...
    private static function is_report_noise_term(string $term, bool $is_hashtag = false): bool {
        ...
        return in_array($term, $platform_noise, true);
    }
