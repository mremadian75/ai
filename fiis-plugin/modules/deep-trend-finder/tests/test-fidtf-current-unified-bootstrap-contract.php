...
// 4. FIDTF_VERSION equals the current module version 0.3.53 (whitespace-tolerant).
$ok((bool) preg_match("/define\\(\\s*'FIDTF_VERSION'\\s*,\\s*'0\\.3\\.53'\\s*\\)/", $src),
    'FIDTF_VERSION equals 0.3.53 in the unified bootstrap');
...
$ok(strpos($src, 'Version: 0.3.53') === false,
    'no DTF test should depend on a standalone DTF plugin header (suite header is the suite version, not 0.3.53)');
