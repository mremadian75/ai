...
final class VES_Access_Control_Admin {
                    ...
                    <fieldset style="border:1px solid #ccd0d4;padding:14px 16px;margin:18px 0;background:#fff">
                        ...
                        <?php foreach ([
                            ...
                            <table class="widefat striped" style="margin-bottom:8px">
                                ...
                                ?>
                                    <tr>
                                        <td><strong><?php echo esc_html(self::human_key_label((string) $item_key)); ?></strong><br><small style="color:#6b7280">key: <code><?php echo esc_html($item_key); ?></code></small></td>
                                        <td>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr>
                        ...
                        <td><textarea name="override_json" id="override-json" class="large-text code" rows="5" placeholder='{"modules":{"ads":"enabled"},"limits":{"monthly_credits":5000}}'></textarea>
                            <p class="description">Advanced operator field. Use human-readable sections above first; this JSON override is for exceptional user-specific access, numeric limits, temporary expiry, and audited support fixes.</p></td>
                    </tr>
                    <tr>
    ...
    public static function render_overview_page() {
        ...
        $warnings = [];
        if (!$has_openai) { $warnings[] = ['OpenAI API key missing.', 'OpenAI is required for AI analysis/generation. Manual workflows, local records, review screens, reports, and the signed callback simulator can still be tested without it.', 'error']; }
        if (!$has_apify)  { $warnings[] = ['Apify token missing.', 'Apify is required for provider-dispatched social/search runs. Manual workflows, local records, review screens, reports, and the signed callback simulator can still be tested without it.', 'error']; }
    ...
    public static function render_providers_page() {
        ...
        <div class="wrap">
            ...
            <p class="description">Human labels are shown first for operator readability; raw provider keys remain visible as mono metadata for debugging. Token values are never rendered.</p>
            <table class="widefat striped" style="max-width:920px">
                <thead><tr><th>Provider</th><th>Module / use case</th><th>Token</th></tr></thead>
                <tbody>
                <?php
                $provider_modules = [
                    'tiktok'        => 'Social Signal Intelligence, Trend Intelligence',
                    'instagram'     => 'Social Signal Intelligence',
                    'youtube'       => 'Social Signal Intelligence, Trend Intelligence',
                    'reddit'        => 'Social Signal Intelligence, Trend Intelligence',
                    'twitter'       => 'Social Signal Intelligence, Trend Intelligence',
                    'facebook'      => 'Social Signal Intelligence',
                    'pinterest'     => 'Social Signal Intelligence',
                    'linkedin'      => 'LinkedIn analysis / manual evidence',
                    ...
                    'semrush'       => 'SEO / competitor intelligence',
                    'moz'           => 'SEO / authority context',
                    'ahrefs'        => 'SEO / backlink context',
                ];
                foreach ($providers as $p):
                    $requires_apify = in_array($p, ['tiktok','instagram','youtube','reddit','twitter','facebook','pinterest','linkedin','google_search','google_trends','google_news','google_ads','meta_ads','semrush'], true);
                    $token_status = $requires_apify ? ($has_apify ? 'configured' : 'missing') : 'optional / external';
                ?>
                    <tr>
                        <td><strong><?php echo esc_html(self::human_key_label((string) $p)); ?></strong><br><small style="color:#6b7280">key: <code><?php echo esc_html($p); ?></code></small></td>
                        <td><?php echo esc_html($provider_modules[$p] ?? 'Custom / future provider'); ?></td>
    ...
    private static function human_key_label(string $key): string {
        $known = [
