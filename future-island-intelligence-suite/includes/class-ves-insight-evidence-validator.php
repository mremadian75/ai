...
final class VES_Insight_Evidence_Validator {
    ...
    public static function validate_claim_support(string $claim, array $evidence_records, array $opts = []) {
        ...
        foreach ($evidence_records as $e) {
            $eid = (int) ($e['id'] ?? 0);
            if ($eid <= 0) { continue; } // a supported claim requires a concrete evidence id (never inferred/memory)
            $has_score = isset($e['confidence_score']) && is_numeric($e['confidence_score']);
            $conf = $has_score ? (float) $e['confidence_score'] : -1.0; // -1 = no score recorded
            // Weak / zero / unscored confidence cannot establish support via a bare
            // term match (would let any keyword-containing text "support" a claim).
            $credible = $has_score && $conf >= $min_conf;
            ...
            // additionally be backed by credible (>= min_confidence) evidence.
    ...
    public static function summarize_evidence(array $evidence_records): array {
        ...
        foreach ($evidence_records as $e) {
            // "Strong" means a CONFIRMED, non-weak evidence item: an explicit
            // confidence_score >= MIN. An explicit 0 (no support) or a missing score
            // is NOT counted as strong, so has_strong is a real evidence-first gate.
