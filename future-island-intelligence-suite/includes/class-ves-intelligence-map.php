...
final class VES_Intelligence_Map {
    ...
    public static function assemble_graph(array $sets, array $args = []): array {
        ...
        $lineage = self::lineage_rows($node_list, $edges);
    ...
    private static function lineage_rows(array $nodes, array $edges): array {
        $by_id = [];
        foreach ($nodes as $node) {
            if (is_array($node) && !empty($node['id'])) { $by_id[(string) $node['id']] = $node; }
        }
        $runs = [];
        foreach ($nodes as $node) {
            if (($node['type'] ?? '') === 'run') {
                $runs[(string) $node['id']] = ['run' => $node, 'insights' => [], 'briefs' => [], 'memory' => []];
            }
        }
        foreach ($edges as $edge) {
            $from = $by_id[(string) ($edge['from'] ?? '')] ?? null;
            $to = $by_id[(string) ($edge['to'] ?? '')] ?? null;
            if (!$from || !$to) { continue; }
            $run_id = '';
            $other = null;
            if (($to['type'] ?? '') === 'run') { $run_id = (string) $to['id']; $other = $from; }
            elseif (($from['type'] ?? '') === 'run') { $run_id = (string) $from['id']; $other = $to; }
            if ($run_id === '' || !isset($runs[$run_id]) || !is_array($other)) { continue; }
            $type = (string) ($other['type'] ?? '');
            if ($type === 'insight') { $runs[$run_id]['insights'][(string) $other['id']] = $other; }
            elseif ($type === 'brief') { $runs[$run_id]['briefs'][(string) $other['id']] = $other; }
            elseif ($type === 'memory') { $runs[$run_id]['memory'][(string) $other['id']] = $other; }
        }
        $rows = [];
        foreach ($runs as $run_id => $group) {
            $insights = array_values($group['insights']);
            $briefs = array_values($group['briefs']);
            $memories = array_values($group['memory']);
            $primary_insight = $insights[0] ?? null;
            $primary_brief = $briefs[0] ?? null;
            $primary_memory = $memories[0] ?? null;
            $dates = array_filter([
                (string) ($group['run']['created_at'] ?? ''),
                $primary_insight ? (string) ($primary_insight['created_at'] ?? '') : '',
                $primary_brief ? (string) ($primary_brief['created_at'] ?? '') : '',
                $primary_memory ? (string) ($primary_memory['created_at'] ?? '') : '',
            ]);
            rsort($dates);
            $rows[] = [
                'run' => (string) ($group['run']['label'] ?? $run_id),
                'run_id' => preg_replace('/^run:/', '', $run_id),
                'insight' => $primary_insight ? (string) ($primary_insight['label'] ?? '') : '',
                'insight_count' => count($insights),
                'brief' => $primary_brief ? (string) ($primary_brief['label'] ?? '') : '',
                'brief_count' => count($briefs),
                'memory' => $primary_memory ? (string) ($primary_memory['label'] ?? '') : '',
                'memory_count' => count($memories),
                'status' => (string) (($primary_insight['status'] ?? '') ?: ($group['run']['status'] ?? '')),
                'confidence' => $primary_insight['confidence'] ?? ($group['run']['confidence'] ?? null),
                'last_updated' => $dates[0] ?? '',
                'open_ref' => $primary_insight['object_ref'] ?? ($primary_brief['object_ref'] ?? ($primary_memory['object_ref'] ?? ($group['run']['object_ref'] ?? null))),
            ];
        }
        usort($rows, static function ($a, $b) { return strcmp((string) ($b['last_updated'] ?? ''), (string) ($a['last_updated'] ?? '')); });
