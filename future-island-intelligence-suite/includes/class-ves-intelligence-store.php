...
final class VES_Intelligence_Store {
    ...
    private static function enums(): array {
        return [
            ...
            'brief_status'  => ['draft', 'ready', 'reviewed', 'approved', 'needs_revision', 'rejected', 'archived'],
            ...
            'memory_type'   => ['run_summary', 'research_summary', 'insight_summary', 'brief_context', 'draft_summary', 'asset_summary', 'reusable_context', 'preference_snapshot', 'review_learning', 'brand_rule', 'failed_assumption', 'campaign_learning', 'social_learning', 'creative_learning', 'aeo_learning', 'source_coverage_learning', 'brand', 'audience', 'competitor', 'performance', 'preference', 'constraint', 'learning', 'other'],
            'memory_status' => ['candidate', 'active', 'pinned', 'expired', 'archived', 'rejected', 'superseded', 'deleted'],
        ];
    }
    ...
    private static function schemas(): array {
        ...
        $t[] = "CREATE TABLE " . self::table_name('brief') . " (
            ...
            origin_type varchar(40) NOT NULL DEFAULT '',
            ...
            decision_context_json longtext NULL,
            prompt_preset_key varchar(120) NOT NULL DEFAULT '',
            ...
            KEY origin (origin_type, origin_id),
    ...
    private static function field_specs(): array {
        return [
            ...
            'brief' => [
                ...
                'origin_type'       => ['type' => 'key', 'max' => 40],
                'origin_id'         => ['type' => 'idref'],
                'brief_type'        => ['type' => 'enum', 'in' => 'brief_type', 'required' => true],
                'channel'           => ['type' => 'text', 'max' => 80],
                'format'            => ['type' => 'text', 'max' => 80],
                'title'             => ['type' => 'text', 'max' => 255, 'required' => true],
                'objective'         => ['type' => 'longtext', 'required' => true],
                'audience'          => ['type' => 'text', 'max' => 255],
                'target_audience'   => ['type' => 'text', 'max' => 255],
                'angle'             => ['type' => 'text', 'max' => 255],
                'key_message'       => ['type' => 'longtext'],
                'key_points_json'   => ['type' => 'json_value', 'input' => 'key_points'],
                'constraints_json'  => ['type' => 'json_value', 'input' => 'constraints'],
                'source_evidence_json' => ['type' => 'json_value', 'input' => 'source_evidence'],
                'decision_context_json' => ['type' => 'json_value', 'input' => 'decision_context'],
                'prompt_preset_key' => ['type' => 'key', 'max' => 120],
                'version'           => ['type' => 'count'],
    ...
    public static function create_record(string $entity, array $data) {
        ...
        if (!array_key_exists('run_id', $data) && array_key_exists('canonical_run_id', $data)) {
            $data['run_id'] = $data['canonical_run_id'];
        }
        $row = ['workspace_id' => $workspace_id, 'run_id' => max(0, (int) ($data['run_id'] ?? 0))];
        $formats = ['%d', '%d'];
    ...
    public static function create_memory_record(array $d) {
        ...
        if (class_exists('VES_Memory_Records') && method_exists('VES_Memory_Records', 'save_record')) {
            $id = (int) VES_Memory_Records::save_record([
                ...
                'expires_at'        => self::sanitize_datetime($d['expires_at'] ?? null),
                'is_pinned'         => !empty($d['is_pinned']) || $status === 'pinned',
                'content'          => ['_canonical_status' => $status, '_canonical_memory_type' => $memory_type, 'run_id' => max(0, (int) ($d['run_id'] ?? 0)), 'trust_label' => self::memory_trust_label($status, (array) ($d['metadata'] ?? []))],
            ]);
        ...
        $ok = $wpdb->insert(self::memory_fallback_table(), [
            ...
            'expires_at'         => self::sanitize_datetime($d['expires_at'] ?? null),
            'is_pinned'          => (!empty($d['is_pinned']) || $status === 'pinned') ? 1 : 0,
            'metadata'           => self::prepare_metadata(array_merge(is_array($d['metadata'] ?? null) ? $d['metadata'] : [], ['trust_label' => self::memory_trust_label($status, is_array($d['metadata'] ?? null) ? $d['metadata'] : [])])),
            'created_at'         => $now,
            'updated_at'         => $now,
        ], ['%d','%d','%s','%s','%f','%s','%s','%s','%s','%d','%s','%s','%s']);
    ...
    /** Update canonical memory trust/status without deleting the memory record. */
    public static function update_memory_status(int $id, string $status, array $metadata_merge = []) {
        global $wpdb;
        $status = self::sanitize_key($status, 24);
        if (!in_array($status, self::enums()['memory_status'], true)) {
            return new WP_Error('ves_intel_invalid_enum', "Invalid memory status '{$status}'.");
        }
        if ($id <= 0 || !isset($wpdb) || !is_object($wpdb) || !method_exists($wpdb, 'get_row')) {
            return new WP_Error('ves_intel_not_found', 'Memory record not found.');
        }
        $current = self::get_memory_record($id);
        if (!is_array($current)) { return new WP_Error('ves_intel_not_found', 'Memory record not found.'); }
        $from = self::sanitize_key((string) ($current['status'] ?? 'candidate'), 24);
        $allowed = class_exists('VES_Review_Decision_Ledger') && method_exists('VES_Review_Decision_Ledger', 'transition_allowed')
            ? VES_Review_Decision_Ledger::transition_allowed('memory_record', $from, $status)
            : ($from === $status);
        if (!$allowed) {
            if (class_exists('VES_Security_Event_Log')) {
                VES_Security_Event_Log::record('memory_transition_blocked', 'Memory status transition blocked.', ['memory_id' => $id, 'from' => $from, 'to' => $status]);
            }
            return new WP_Error('ves_intel_transition_blocked', "Memory status transition '{$from}' -> '{$status}' is not allowed.", ['from' => $from, 'to' => $status]);
        }

        $from_existing = class_exists('VES_Memory_Records') && method_exists('VES_Memory_Records', 'table_name');
        $table = $from_existing ? VES_Memory_Records::table_name() : self::memory_fallback_table();
        $row = $wpdb->get_row($wpdb->prepare('SELECT * FROM ' . $table . ' WHERE id = %d', $id), ARRAY_A);
        if (!is_array($row)) { return new WP_Error('ves_intel_not_found', 'Memory record not found.'); }
        $column = $from_existing ? 'content_json' : 'metadata';
        $payload = json_decode((string) ($row[$column] ?? ''), true);
        $payload = is_array($payload) ? $payload : [];
        foreach ($metadata_merge as $k => $v) { if (is_scalar($v) || is_array($v) || $v === null) { $payload[(string) $k] = $v; } }
        $payload['_canonical_status'] = $status;
        $payload['trust_label'] = self::memory_trust_label($status, $payload);
        $payload['status_updated_at'] = function_exists('current_time') ? current_time('mysql', true) : gmdate('Y-m-d H:i:s');
        $payload['status_updated_by'] = function_exists('get_current_user_id') ? (int) get_current_user_id() : 0;
        $update = [$column => wp_json_encode($payload)];
        if (!$from_existing) { $update['status'] = $status; }
        if (array_key_exists('is_pinned', $row)) { $update['is_pinned'] = $status === 'pinned' ? 1 : (!empty($metadata_merge['is_pinned']) ? 1 : 0); }
        if (array_key_exists('expires_at', $row) && !empty($metadata_merge['expires_at'])) { $update['expires_at'] = self::sanitize_datetime($metadata_merge['expires_at']); }
        if (array_key_exists('updated_at', $row)) { $update['updated_at'] = function_exists('current_time') ? current_time('mysql', true) : gmdate('Y-m-d H:i:s'); }
        $ok = $wpdb->update($table, $update, ['id' => $id]);
        if ($ok === false) { return new WP_Error('ves_intel_memory_failed', 'Memory status could not be saved.'); }
        if ($from !== $status && class_exists('VES_Review_Decision_Ledger')) {
            $decision = ['active' => 'approve', 'pinned' => 'pin', 'rejected' => 'reject', 'archived' => 'archive', 'expired' => 'mark_stale'][$status] ?? 'status_change';
            VES_Review_Decision_Ledger::record([
                'workspace_id' => (int) ($current['workspace_id'] ?? 0),
                'object_type' => 'memory_record',
                'object_id' => $id,
                'from_status' => $from,
                'to_status' => $status,
                'decision' => $decision,
                'reason' => is_scalar($metadata_merge['reason'] ?? null) ? (string) $metadata_merge['reason'] : '',
                'reason_code' => is_scalar($metadata_merge['reason_code'] ?? null) ? (string) $metadata_merge['reason_code'] : '',
                'metadata' => ['source' => 'memory_status_update'],
            ]);
        }
        return ['memory_id' => $id, 'from_status' => $from, 'status' => $status, 'trust_label' => (string) ($payload['trust_label'] ?? '')];
    }

    private static function memory_trust_label(string $status, array $metadata = []): string {
        $approval = (string) ($metadata['approval_status'] ?? '');
        if ($status === 'pinned' || !empty($metadata['is_pinned'])) { return 'pinned approved context - not evidence'; }
        if ($approval === 'approved' || $status === 'active') { return 'approved context - not evidence'; }
        if ($approval === 'rejected' || $status === 'rejected') { return 'rejected memory - do not use'; }
        if ($status === 'expired') { return 'expired context - verify before use'; }
        if ($status === 'archived') { return 'archived context - do not use'; }
        return 'candidate context - not evidence';
    }
    ...
    private static function normalize_memory_row(array $row, bool $from_existing): array {
        if ($from_existing) {
            ...
            return [
                ...
                'is_pinned'          => !empty($row['is_pinned']) || $status === 'pinned',
                'trust_label'        => self::memory_trust_label($status, $content),
        ...
        return [
            ...
            'trust_label'        => self::memory_trust_label((string) ($row['status'] ?? 'active'), is_array($meta) ? $meta : []),
            'created_at'         => (string) ($row['created_at'] ?? ''),
            'updated_at'         => (string) ($row['updated_at'] ?? ''),
    ...
    public static function ingest_bundle(array $bundle): array {
        ...
        $with_ws = function (array $d) use ($ws, $bundle) {
            ...
            if (!isset($d['run_id']) && isset($bundle['run_id'])) { $d['run_id'] = max(0, (int) $bundle['run_id']); }
            return $d;
        };
    ...
    /** Attach a canonical run id to an existing canonical record. Additive helper for legacy flows. */
    public static function attach_run_id(string $entity, int $id, int $run_id) {
        global $wpdb;
        if (!in_array($entity, self::entities(), true)) { return new WP_Error('ves_intel_unknown_entity', 'Unknown intelligence entity: ' . $entity); }
        if ($id <= 0 || $run_id <= 0) { return new WP_Error('ves_intel_bad_link', 'Invalid link ids.'); }
        if (!isset($wpdb) || !is_object($wpdb)) { return new WP_Error('ves_intel_no_db', 'Database unavailable.'); }
        $row = self::get($entity, $id);
        if (!is_array($row)) { return new WP_Error('ves_intel_not_found', ucfirst($entity) . ' not found.'); }
        $now = function_exists('current_time') ? current_time('mysql', true) : gmdate('Y-m-d H:i:s');
        $updated = $wpdb->update(self::table_name($entity), ['run_id' => $run_id, 'updated_at' => $now], ['id' => $id], ['%d','%s'], ['%d']);
        return $updated === false ? new WP_Error('ves_intel_link_failed', 'Could not attach run id.') : true;
    }
    ...
    private static function record_review_decision(string $object_type, array $row, string $from, string $to, array $metadata_merge, array $evidence_ids, array $opts = []): void {
        ...
        $res = VES_Review_Decision_Ledger::record([
            ...
            'decision' => $decision,
            'reason' => $reason,
            'reason_code' => isset($metadata_merge['reason_code']) ? (string) $metadata_merge['reason_code'] : (isset($opts['reason_code']) ? (string) $opts['reason_code'] : ''),
    ...
    private static function coerce_field(string $type, $raw, array $spec, string $entity, string $key): array {
        switch ($type) {
            ...
            case 'json_value':
                return [self::prepare_json_value($raw), '%s', null];
            default:
    ...
    private static function prepare_json_value($value): string {
        $clean = self::sanitize_json_value($value, 0);
    ...
    private static function sanitize_json_value($value, int $depth) {
        ...
        if (is_array($value)) {
            ...
            foreach ($value as $k => $v) {
                if ($i++ >= 50) { break; }
                $key = is_int($k) ? $k : self::sanitize_key((string) $k, 80);
                if ($key === '') { continue; }
                $out[$key] = self::sanitize_json_value($v, $depth + 1);
            }
            return $out;
        }
        if (is_bool($value) || is_int($value) || is_float($value) || $value === null) { return $value; }
        return self::scrub_secrets(self::sanitize_text((string) $value, 1000));
    }

    /** Decode JSON columns into arrays for callers. */
    private static function decode_row(array $row): array {
        foreach (['id', 'workspace_id', 'run_id', 'source_id', 'signal_id', 'insight_id', 'brief_id'] as $id_col) {
            if (array_key_exists($id_col, $row)) { $row[$id_col] = (int) $row[$id_col]; }
        }
        ...
        foreach (['key_points_json' => 'key_points', 'constraints_json' => 'constraints', 'source_evidence_json' => 'source_evidence', 'decision_context_json' => 'decision_context'] as $col => $alias) {
