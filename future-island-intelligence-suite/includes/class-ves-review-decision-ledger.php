...
final class VES_Review_Decision_Ledger {
    ...
    const OBJECT_TYPES = ['insight', 'brief', 'draft', 'memory_record', 'prompt_package'];
    const DECISIONS = ['approve', 'reject', 'archive', 'reopen', 'restore', 'pin', 'unpin', 'mark_needs_revision', 'promote_to_reviewed', 'status_change'];
    ...
    public static function record(array $args) {
        ...
        $row['metadata'] = self::encode_metadata(is_array($args['metadata'] ?? null) ? $args['metadata'] : []);
        $ok = $wpdb->insert(self::table_name(), $row, ['%d','%s','%d','%s','%s','%s','%d','%s','%s','%s','%s','%s']);
    ...
    public static function transition_allowed($object_type, $from, $to) {
        ...
        if ($object_type === 'memory_record') {
            ...
            $matrix = [
                ...
                'pinned'    => ['pinned', 'active', 'rejected', 'archived'],
                'rejected'  => ['rejected'],
                'archived'  => ['archived'],
    ...
    private static function encode_metadata(array $metadata) {
        $clean = [];
        $forbidden = ['raw_prompt', 'prompt_text', 'raw_response', 'provider_response', 'api_key', 'apikey', 'token', 'authorization', 'bearer', 'secret', 'password'];
        $i = 0;
