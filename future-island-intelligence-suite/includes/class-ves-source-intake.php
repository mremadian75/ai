...
final class VES_Source_Intake {
    ...
    /** Create a canonical Run when the additive run service is available. */
    private static function begin_run($ws, string $run_type, string $trigger_type, int $source_id, array $context, string $seed = '', int $parent_run_id = 0): int {
        $ws = max(0, (int) $ws);
        if ($ws <= 0 || !class_exists('VES_Run_Service')) { return 0; }
        $seed = $seed !== '' ? $seed : (function_exists('wp_json_encode') ? wp_json_encode($context) : json_encode($context));
        if (!is_string($seed)) { $seed = ''; }
        $idempotency_key = substr(hash('sha256', 'intake|' . $ws . '|' . $run_type . '|' . $trigger_type . '|' . max(0, $source_id) . '|' . $seed), 0, 64);
        $run_id = VES_Run_Service::create_run([
            ...
            'parent_run_id' => $parent_run_id > 0 ? $parent_run_id : null,
            'idempotency_key' => $idempotency_key,
            'initiated_by_user_id' => function_exists('get_current_user_id') ? (int) get_current_user_id() : 0,
            'metadata' => ['created_via' => 'source_intake'],
        ]);
        if (self::is_err($run_id) || (int) $run_id <= 0) { return 0; }
        $run_id = (int) $run_id;
        $mark = VES_Run_Service::mark_running($run_id);
        return self::is_err($mark) ? $run_id : $run_id;
    }

    private static function close_run(int $run_id, string $status = 'completed', array $summary = [], string $error_summary = ''): void {
        if ($run_id <= 0 || !class_exists('VES_Run_Service')) { return; }
        if ($status === 'failed') { VES_Run_Service::mark_failed($run_id, $error_summary !== '' ? $error_summary : 'Intake run failed.', $summary); return; }
        if ($status === 'partial') { VES_Run_Service::mark_partial($run_id, $summary); return; }
        if ($status === 'timed_out') { VES_Run_Service::mark_timed_out($run_id, $summary); return; }
        if ($status === 'canceled') { VES_Run_Service::cancel_run($run_id); return; }
        VES_Run_Service::mark_completed($run_id, $summary);
    }

    private static function edge($ws, int $run_id, string $from_type, int $from_id, string $to_type, int $to_id, string $relation_type, array $args = []): void {
        if ($run_id <= 0 || $from_id <= 0 || $to_id <= 0 || !class_exists('VES_Decision_Edge_Service')) { return; }
        VES_Decision_Edge_Service::add_edge((int) $ws, $run_id, $from_type, $from_id, $to_type, $to_id, $relation_type, $args);
    }

    private static function row_run_id(array $row): int {
        $run_id = max(0, (int) ($row['run_id'] ?? 0));
        if ($run_id > 0) { return $run_id; }
        $meta = $row['metadata'] ?? [];
        if (is_array($meta)) { return max(0, (int) ($meta['run_id'] ?? 0)); }
        return 0;
    }
    ...
    public static function process_source(array $in) {
        ...
        $run_id = self::begin_run($ws, 'research', 'manual', 0, [
            'intake_type' => $type,
            'source_title' => (string) ($payload['source_title'] ?? $title),
            'has_url' => !empty($payload['source_url']),
        ], $type . '|' . (string) ($payload['source_url'] ?? '') . '|' . $title . '|' . substr(hash('sha256', $notes), 0, 16));
        if ($run_id > 0) {
            $payload['run_id'] = $run_id;
            $payload['metadata']['run_id'] = $run_id;
        }
        $id = VES_Intelligence_Store::create_or_get_source($payload);
        if (self::is_err($id)) { self::close_run($run_id, 'failed', ['stage' => 'source'], $id->get_error_message()); return $id; }
        self::edge($ws, $run_id, 'source', (int) $id, 'run', $run_id, 'generated', ['metadata' => ['created_via' => 'intake_source']]);
        self::close_run($run_id, 'completed', ['source_id' => (int) $id]);
        return ['source_id' => (int) $id, 'run_id' => $run_id];
    }
    ...
    public static function process_signal(array $in) {
        ...
        $run_id = self::begin_run($ws, 'research', 'manual', (int) $source['id'], [
            'stage' => 'signal_capture',
            'source_id' => (int) $source['id'],
            'signal_type' => $stype,
            'title' => $title,
        ], 'signal|' . (int) $source['id'] . '|' . $stype . '|' . $title);
        ...
        if (self::is_err($id)) { self::close_run($run_id, 'failed', ['stage' => 'signal'], $id->get_error_message()); return $id; }
        self::edge($ws, $run_id, 'source', (int) $source['id'], 'run', $run_id, 'generated', ['metadata' => ['stage' => 'signal_capture']]);
        self::edge($ws, $run_id, 'run', $run_id, 'signal', (int) $id, 'captured', ['metadata' => ['source_id' => (int) $source['id']]]);
        self::close_run($run_id, 'completed', ['source_id' => (int) $source['id'], 'signal_id' => (int) $id]);
        return ['signal_id' => (int) $id, 'run_id' => $run_id];
    }
    ...
    public static function process_signal_to_insight(array $in) {
        ...
        $parent_run_id = self::row_run_id($signal);
        $run_id = self::begin_run($ws, 'research', 'manual', (int) ($signal['source_id'] ?? 0), [
            'stage' => 'signal_to_insight',
            'signal_id' => (int) $signal['id'],
            'insight_type' => $itype,
            'title' => $title,
        ], 'signal_to_insight|' . (int) $signal['id'] . '|' . $itype . '|' . $title, $parent_run_id);
        ...
        $evidence_id = VES_Intelligence_Store::create_evidence([
            ...
            'metadata'        => ['created_via' => 'intake_promotion', 'signal_id' => (int) $signal['id'], 'run_id' => $run_id],
        ]);
        if (self::is_err($evidence_id)) { self::close_run($run_id, 'failed', ['stage' => 'evidence'], $evidence_id->get_error_message()); return $evidence_id; }

        $meta = ['created_via' => 'intake_promotion', 'source_signal_id' => (int) $signal['id'], 'run_id' => $run_id];
        ...
        if (self::is_err($insight_id)) { self::close_run($run_id, 'failed', ['stage' => 'insight', 'evidence_id' => (int) $evidence_id], $insight_id->get_error_message()); return $insight_id; }
        self::edge($ws, $run_id, 'run', $run_id, 'evidence', (int) $evidence_id, 'captured', ['metadata' => ['signal_id' => (int) $signal['id']]]);
        self::edge($ws, $run_id, 'signal', (int) $signal['id'], 'evidence', (int) $evidence_id, 'supports');
        self::edge($ws, $run_id, 'evidence', (int) $evidence_id, 'insight', (int) $insight_id, 'supports');
        self::edge($ws, $run_id, 'signal', (int) $signal['id'], 'insight', (int) $insight_id, 'supports');
        self::close_run($run_id, 'completed', ['signal_id' => (int) $signal['id'], 'evidence_id' => (int) $evidence_id, 'insight_id' => (int) $insight_id]);
        return ['evidence_id' => (int) $evidence_id, 'insight_id' => (int) $insight_id, 'run_id' => $run_id];
    }
    ...
    public static function process_insight_to_brief(array $in) {
        ...
        $parent_run_id = self::row_run_id($insight);
        $run_id = self::begin_run($ws, 'creation', 'manual', 0, [
            'stage' => 'insight_to_brief',
            'insight_id' => $insight_id,
        ], 'insight_to_brief|' . $insight_id, $parent_run_id);
        $brief_id = VES_Insight_Brief_Builder::create_brief_from_insight($insight_id, ['run_id' => $run_id]);
        if (self::is_err($brief_id)) { self::close_run($run_id, 'failed', ['stage' => 'brief'], $brief_id->get_error_message()); return $brief_id; }
        self::edge($ws, $run_id, 'insight', $insight_id, 'brief', (int) $brief_id, 'generated');
        self::close_run($run_id, 'completed', ['insight_id' => $insight_id, 'brief_id' => (int) $brief_id]);
        return ['brief_id' => (int) $brief_id, 'run_id' => $run_id];
    }
    ...
    public static function process_brief_to_draft(array $in) {
        ...
        $parent_run_id = self::row_run_id($brief);
        $run_id = self::begin_run($ws, 'creation', 'manual', 0, [
            'stage' => 'brief_to_draft',
            'brief_id' => $brief_id,
        ], 'brief_to_draft|' . $brief_id, $parent_run_id);

        $existing = self::find_existing_draft_for_brief($ws, $brief_id);
        if ($existing > 0) { self::close_run($run_id, 'completed', ['brief_id' => $brief_id, 'draft_id' => $existing, 'dedupe' => true]); return ['draft_id' => $existing, 'run_id' => $run_id]; }
        ...
        if (self::is_err($draft_id)) { self::close_run($run_id, 'failed', ['stage' => 'draft'], $draft_id->get_error_message()); return $draft_id; }
        self::edge($ws, $run_id, 'brief', $brief_id, 'draft', (int) $draft_id, 'generated');
        self::close_run($run_id, 'completed', ['brief_id' => $brief_id, 'draft_id' => (int) $draft_id]);
        return ['draft_id' => (int) $draft_id, 'run_id' => $run_id];
    }
    ...
    public static function process_draft_to_memory(array $in) {
        ...
        $in_ws = self::assert_in_workspace('draft', $draft, $ws);
        if (self::is_err($in_ws)) { return $in_ws; }
        $parent_run_id = self::row_run_id($draft);
        $run_id = self::begin_run($ws, 'memory_refresh', 'manual', 0, [
            'stage' => 'draft_to_memory',
            'draft_id' => $draft_id,
        ], 'draft_to_memory|' . $draft_id, $parent_run_id);
        ...
        if (!empty($existing[0]['id'])) { self::close_run($run_id, 'completed', ['memory_id' => (int) $existing[0]['id'], 'dedupe' => true]); return ['memory_id' => (int) $existing[0]['id'], 'run_id' => $run_id]; }
        ...
        $memory_id = VES_Intelligence_Store::create_memory_record([
            ...
            // Memory is continuity context, never evidence. Approval must happen before trusted reuse.
            'metadata' => ['created_via' => 'intake_draft_to_memory', 'requires_review' => true, 'approval_status' => 'pending_review', 'run_id' => $run_id],
        ]);
        if (self::is_err($memory_id)) { self::close_run($run_id, 'failed', ['stage' => 'memory'], $memory_id->get_error_message()); return $memory_id; }
        self::edge($ws, $run_id, 'run', $run_id, 'memory_record', (int) $memory_id, 'created_memory', ['metadata' => ['source_draft_id' => $draft_id]]);
        self::close_run($run_id, 'completed', ['draft_id' => $draft_id, 'memory_id' => (int) $memory_id]);
        return ['memory_id' => (int) $memory_id, 'run_id' => $run_id];
    }

    /** @return array{usage_event_id:int}|WP_Error */
    public static function process_record_usage_event(array $in) {
        if (!class_exists('VES_Intelligence_Store')) { return self::err('ves_intake_store_missing', 'Intelligence store unavailable.'); }
        if (!class_exists('VES_Usage_Service') && !class_exists('VES_AI_Usage_Tracker')) { return self::err('ves_intake_usage_tracker_missing', 'Usage tracker unavailable.'); }
        $ws = self::assert_workspace($in['workspace_id'] ?? 0);
        if (self::is_err($ws)) { return $ws; }
        $draft_id = (int) ($in['draft_id'] ?? 0);
        $draft = $draft_id > 0 ? VES_Intelligence_Store::get_draft($draft_id) : null;
        ...
        $run_id = self::row_run_id($draft);
        if ($run_id <= 0) {
            $run_id = self::begin_run($ws, 'creation', 'manual', 0, [
                'stage' => 'usage_event',
                'draft_id' => $draft_id,
            ], 'usage_event|' . $draft_id);
        }

        $transient_key = 'ves_intake_usage_' . $ws . '_' . $draft_id . '_' . $run_id;
        ...
        if (class_exists('VES_Usage_Service')) {
            $event_id = VES_Usage_Service::record_zero_cost_event([
                'workspace_id' => $ws,
                'run_id' => $run_id,
                'target_type' => 'draft',
                'target_id' => (string) $draft_id,
                'event_type' => 'approved_output_usage_event',
                'module' => 'future_island_intake',
                'user_id' => function_exists('get_current_user_id') ? (int) get_current_user_id() : 0,
                'idempotency_key' => 'fi:intake:usage:' . $ws . ':' . $run_id . ':' . $draft_id,
                'message' => 'Approved output usage event',
                'context' => ['created_via' => 'intake_approved_output_usage_event'],
            ]);
            if (self::is_err($event_id)) { self::close_run($run_id, 'failed', ['stage' => 'usage'], $event_id->get_error_message()); return $event_id; }
            ...
            $event_id = (int) VES_AI_Usage_Tracker::record([
                ...
                'run_id' => $run_id > 0 ? (string) $run_id : ('fi-intake-draft-' . $draft_id),
                'provider' => 'manual',
        ...
        if ($event_id <= 0) { self::close_run($run_id, 'failed', ['stage' => 'usage'], 'Usage event could not be recorded.'); return self::err('ves_intake_usage_record_failed', 'Usage event could not be recorded.'); }
        self::edge($ws, $run_id, 'run', $run_id, 'usage_event', $event_id, 'spent_usage', ['metadata' => ['target_type' => 'draft', 'target_id' => $draft_id, 'zero_cost' => true]]);
        if (function_exists('set_transient')) {
            $ttl = 7 * (defined('DAY_IN_SECONDS') ? (int) DAY_IN_SECONDS : 86400);
            set_transient($transient_key, $event_id, $ttl);
        }
        self::close_run($run_id, 'completed', ['draft_id' => $draft_id, 'usage_event_id' => $event_id, 'zero_cost' => true]);
        return ['usage_event_id' => $event_id, 'run_id' => $run_id];
    }
    ...
    private static function recent_objects($ws) {
        ...
        if (count($sources) === 0) { $h .= '<p class="fi-queue-hint">' . self::e('No sources yet — record one above.') . '</p>'; }
            ...
            $h .= '<table class="widefat striped"><thead><tr><th>id</th><th>run</th><th>type</th><th>title</th><th>reference</th><th>next action</th></tr></thead><tbody>';
            foreach ($sources as $s) {
                $id = (int) ($s['id'] ?? 0);
                $url = (string) ($s['source_url'] ?? '');
                $use_url = self::page_url($ws, ['prefill_source_id' => $id]) . '#fi-intake-signal';
                $run_cell = self::row_run_id($s) > 0 ? '#' . self::row_run_id($s) : '—';
                $h .= '<tr><td>' . self::e((string) $id) . '</td><td><span class="fi-intake-mono">' . self::e($run_cell) . '</span></td><td>' . self::e((string) ($s['source_type'] ?? '')) . '</td><td>' . self::e((string) ($s['source_title'] ?? '')) . '</td><td>' . ($url !== '' ? '<span class="fi-intake-mono">' . self::e(self::cut($url, 80)) . '</span>' : self::e('—')) . '</td><td><a class="button button-small" href="' . self::eu($use_url) . '">' . self::e('Use for signal') . '</a></td></tr>';
            }
            $h .= '</tbody></table>';
        }
        ...
        if (count($signals) === 0) { $h .= '<p class="fi-queue-hint">' . self::e('No signals yet — a signal records what a source showed.') . '</p>'; }
            ...
            $h .= '<table class="widefat striped"><thead><tr><th>id</th><th>run</th><th>source</th><th>type</th><th>title</th><th>seen</th><th>next action</th></tr></thead><tbody>';
            foreach ($signals as $s) {
                ...
                $run_cell = self::row_run_id($s) > 0 ? '#' . self::row_run_id($s) : '—';
                $h .= '<tr><td>' . self::e((string) $id) . '</td><td><span class="fi-intake-mono">' . self::e($run_cell) . '</span></td><td>' . self::e('#' . (int) ($s['source_id'] ?? 0)) . '</td><td>' . self::e((string) ($s['signal_type'] ?? '')) . '</td><td>' . self::e($title) . '</td><td>' . self::e((string) (int) ($s['recurrence_count'] ?? 1) . '×') . '</td><td>' . self::action_form(self::ACTION_INSIGHT, $fields, 'Promote to insight') . '</td></tr>';
            }
            $h .= '</tbody></table>';
        }
        ...
        if (count($insights) === 0) { $h .= '<p class="fi-queue-hint">' . self::e('No insights yet — promote a signal when a finding emerges.') . '</p>'; }
            ...
            $h .= '<table class="widefat striped"><thead><tr><th>id</th><th>run</th><th>type</th><th>title</th><th>state</th><th>evidence</th><th>next action</th></tr></thead><tbody>';
            foreach ($insights as $i) {
                ...
                $run_cell = self::row_run_id($i) > 0 ? '#' . self::row_run_id($i) : '—';
                $h .= '<tr><td>' . self::e((string) $id) . '</td><td><span class="fi-intake-mono">' . self::e($run_cell) . '</span></td><td>' . self::e((string) ($i['insight_type'] ?? '')) . '</td><td>' . self::e((string) ($i['title'] ?? '')) . '</td><td>' . $badge($pstate) . '</td><td>' . self::e($evi . ' linked') . '</td><td>' . $wb . $action . '</td></tr>';
            }
            $h .= '</tbody></table>';
        }
        ...
        if (count($briefs) === 0) { $h .= '<p class="fi-queue-hint">' . self::e('No briefs yet — approve an insight, then build its brief above.') . '</p>'; }
            ...
            $h .= '<table class="widefat striped"><thead><tr><th>id</th><th>run</th><th>insight</th><th>title</th><th>state</th><th>next action</th></tr></thead><tbody>';
            foreach ($briefs as $b) {
                $id = (int) ($b['id'] ?? 0);
                ...
                $run_cell = self::row_run_id($b) > 0 ? '#' . self::row_run_id($b) : '—';
                $h .= '<tr><td>' . self::e((string) $id) . '</td><td><span class="fi-intake-mono">' . self::e($run_cell) . '</span></td><td>' . self::e('#' . (int) ($b['insight_id'] ?? 0)) . '</td><td>' . self::e((string) ($b['title'] ?? '')) . '</td><td>' . $badge((string) ($b['status'] ?? 'draft')) . '</td><td>' . $wb . $action . '</td></tr>';
            }
            $h .= '</tbody></table>';
        }
        ...
        if (count($drafts) === 0) { $h .= '<p class="fi-queue-hint">' . self::e('No drafts yet — generate a draft block from a brief.') . '</p>'; }
            ...
            $h .= '<table class="widefat striped"><thead><tr><th>id</th><th>run</th><th>brief</th><th>channel</th><th>title</th><th>state</th><th>next action</th></tr></thead><tbody>';
            foreach ($drafts as $d) {
                $id = (int) ($d['id'] ?? 0);
                ...
                $run_cell = self::row_run_id($d) > 0 ? '#' . self::row_run_id($d) : '—';
                $h .= '<tr><td>' . self::e((string) $id) . '</td><td><span class="fi-intake-mono">' . self::e($run_cell) . '</span></td><td>' . self::e('#' . (int) ($d['brief_id'] ?? 0)) . '</td><td>' . self::e((string) ($d['channel'] ?? '')) . '</td><td>' . self::e((string) ($d['title'] ?? '')) . '</td><td>' . $badge($status) . '</td><td>' . $memory . $review . '</td></tr>';
            }
            $h .= '</tbody></table>';
        }
        $h .= '</section>';
