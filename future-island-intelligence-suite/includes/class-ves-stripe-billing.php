...
final class VES_Stripe_Billing {
    ...
    public static function handle_save_settings() {
        ...
        // Only overwrite a stored secret when a NON-EMPTY new value is submitted, so
        // the masked (empty-value) admin field never wipes the saved key on re-save.
        if (isset($_POST['ves_stripe_secret_key'])) {
            $new_sk = sanitize_text_field(wp_unslash($_POST['ves_stripe_secret_key']));
            if ($new_sk !== '') { $s['secret_key'] = $new_sk; }
        }
        if (isset($_POST['ves_stripe_webhook_secret'])) {
            $new_wh = sanitize_text_field(wp_unslash($_POST['ves_stripe_webhook_secret']));
            if ($new_wh !== '') { $s['webhook_secret'] = $new_wh; }
        }
    ...
    public static function render_admin_settings_html() {
        ...
        if ($key_via_const) {
            ...
            // SECURITY: never echo the stored secret back into the DOM. Show only a
            // masked set/unset hint; an empty submit leaves the saved value intact.
            $sk_set = (string) ($s['secret_key'] ?? '') !== '';
            echo '<p><label>Clave secreta (sk_test_… / sk_live_…)<br><input type="password" name="ves_stripe_secret_key" class="regular-text" autocomplete="off" value="" placeholder="' . esc_attr($sk_set ? '•••••• guardada — déjalo vacío para conservar' : 'sk_live_…') . '" style="max-width:480px;width:100%"></label></p>';
        }
        if ($hook_via_const) {
            ...
            echo '<p><label>Secreto de firma del webhook (whsec_…)<br><input type="password" name="ves_stripe_webhook_secret" class="regular-text" autocomplete="off" value="" placeholder="' . esc_attr($wh_set ? '•••••• guardado — déjalo vacío para conservar' : 'whsec_…') . '" style="max-width:480px;width:100%"></label></p>';
        }
