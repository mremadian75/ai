...
ok(/Revisión de evidencia/.test(gateHtml), '9b: Evidence Gate title is Spanish-first');
ok(/Confianza/.test(gateHtml) && /baja/.test(gateHtml), '9b: low confidence is rendered as baja');
var routeHtml = P.renderRouteBar(P.build({ confidence: 'medium', evidence_quality: { caption: true, metrics: true } }, { usable_signal_count: 2 }));
ok(/Flujo del objeto/.test(routeHtml) && /Borrador/.test(routeHtml) && /Uso registrado/.test(routeHtml), '9b: route labels use Spanish visible UI');
ok(!/>Draft</.test(routeHtml) && !/>Usage</.test(routeHtml) && !/>Evidence Gate</.test(gateHtml), '9b: stale English visible labels removed from productization block');
