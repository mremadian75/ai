...
// A1b (telemetry honesty fix): a COMPLETED call on an UNKNOWN model (pricing_source
// = approximate/'fallback') must NOT be written as actual_provider_cost; it is
// reported as an estimate. Only confirmed ('known') prices are "actual".
$GLOBALS['wpdb']->inserts = [];
VES_AI_Usage_Tracker::record(['module' => 'unit', 'operation_type' => 'analyze', 'model' => 'totally-unknown-xyz', 'status' => 'completed', 'input_tokens' => 100, 'output_tokens' => 50]);
$rowUnk = $GLOBALS['wpdb']->inserts[0]['row'] ?? [];
$ok(($rowUnk['pricing_source'] ?? '') !== 'known', 'A1b: unknown model resolves a non-known pricing_source');
$ok($approx($rowUnk['actual_provider_cost'], 0.0), 'A1b: unknown-price completed call does NOT fabricate actual_provider_cost (actual=0)');
$ok(($rowUnk['estimated_provider_cost'] ?? 0) > 0, 'A1b: unknown-price completed call reports the figure as ESTIMATED honestly');
