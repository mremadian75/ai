...
(function (root) {
  ...
  var L = {
    ...
    brief: 'Brief candidato', draft: 'Draft', memory: 'Memoria', usage: 'Uso registrado',
    completed: 'completado', current: 'en curso', pending: 'pendiente', blocked: 'bloqueado', not_available: 'no disponible',
    gateTitle: 'Evidence Gate', confidence: 'Confianza', evidenceStatus: 'Estado de evidencia',
    ...
    approve: 'Aprobar insight', createBrief: 'Crear brief', generateDraft: 'Generar draft',
  ...
  function renderEvidenceGate(model) {
    ...
    return '<section class="fiis-evidence-gate is-' + esc(g.status) + ' is-source-' + esc(g.source) + '">' +
      ...
      sourceLine +
      '<div class="fiis-eg-row"><span>' + esc(L.confidence) + '</span><strong class="fiis-conf is-' + esc(g.confidence) + '">' + esc(g.confidence) + '</strong></div>' +
  ...
  function renderRecommendations(recs, model) {
    ...
    var cards = recs.filter(Boolean).map(function (r) {
      var conf = esc(r.confidence || model.gate.confidence || 'low');
      var risk = esc(r.risk || '—');
      var type = esc(r.type || r.recommendation_type || 'acción');
      var basis = esc(r.evidence_basis || r.why || '');
      var nextObj = esc(r.suggested_next || (canBrief ? 'Brief' : 'A/B test'));
      var ctas =
