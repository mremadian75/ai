    ...
    public static function inventory() {
        return [
            ...
            self::row('apify', 'VES_Market_Signal_Commercial', 'run_apify_sync', 'apify_run_start_core_guarded', true,
                ['Phase 9D.2: direct wp_remote_post removed; run-sync routed through VES_Apify_Client::request with ceiling; fail-closed without the core client.']),
            ...
            self::row('openai', 'includes/analysis.php', 'ves_call_openai_analysis (direct post)', 'ai_provider_legacy_requires_review', true,
                ['Legacy direct call for scrape analysis; key + operator action gated; consolidation into VES_OpenAI_Client is roadmap.']),
            self::row('openai', 'VES_Market_Signal_Commercial', 'run_chatgpt (direct post)', 'ai_provider_legacy_requires_review', true,
